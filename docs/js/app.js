App = {
    ...
    displayAccountInfo: async () => {
      ...
      App.count++;
        ...
        $('#account').text("account: "+App.account+"  =============> Balance: ");
        const balance = await window.web3.eth.getBalance(accounts[0]);
        $('#accountBalance').text(window.web3.utils.fromWei(balance, "ether") + " ETH");
        const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
        const totalSupply = await OptimisationGasTestInstance.totalSupply();
        const tokenBalance = await OptimisationGasTestInstance.balanceOf(accounts[0]);
        const tokBlance = window.web3.utils.fromWei(tokenBalance, "ether")
        console.log("token Balance",tokBlance + " BTU");
        $('#totalSupply').text( "Total supply: "+totalSupply + " BTU")
        $('#tokenBalance').text("Token balance: " + window.web3.utils.fromWei(tokenBalance, "wei") + " BTU")
    },

    initContract: async () => {
      console.log("initContract demende===========================>",App.count);
      App.count++;
        $.getJSON('OptimisationGasTest.json', OptimisationGasTestArtifact => {
            App.contracts.OptimisationGasTestArtifact = TruffleContract(OptimisationGasTestArtifact);
            App.contracts.OptimisationGasTestArtifact.setProvider(window.web3.currentProvider);
    ...
    listenToEvents: async () => {
      console.log("listenToEvents demende===========================>");
        ...
        if(App.TransferListener == null) {
            App.TransferListener = OptimisationGasTestInstance
...
//======================================================================================================
  ...
  runScript: async () => {
    ...
    await App.transferETRMethod();
    await App.transfer();
    await App.transferConsenSys();
    await App.sendBatch(10);
    console.log("sendBatch 100 call");
    await App.sendBatch(100);
    console.log("sendBatchCs 10 call");
    await App.sendBatchCS(10);
    console.log("sendBatchCs 100 call");
    await App.sendBatchCS(100);
    console.log("sendBatchCs 1000 call");
    await App.sendBatchCS(500);
  ...
  transfer: async () => {
    $('#opZplin').empty();
    $('#opZplinAverage').empty();
    var index = Math.floor((Math.random() * 1000) + 1);
    console.log(index);
    var _adresandBalanc = await App.getAddresses(index)
    console.log(_adresandBalanc);
    var _from = App.account
    console.log(App.account);
    var _to = _adresandBalanc[0];
    console.log("address to: ",_to[index-1]);
    var _amount = _adresandBalanc[1];
    console.log("Amount: ",_amount[index-1]);

//try {
  const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
  const transactionReceipt = await OptimisationGasTestInstance.transfer(_to[index-1], _amount[index-1]* 100, {from: App.account}
).on("transactionHash", hash => {
    console.log("transaction hash", hash);

});

var gas = transactionReceipt.receipt.gasUsed;
console.log("gasUsed in this transaction: ", gas );
$('#opZplin').text(gas*1000);
$('#opZplinAverage').text(gas);
//} catch(error) {
  //console.log("error transfer");
//}
},
  transferETRMethod: async () => {
    $('#eth').empty();
    $('#ethAverage').empty();
    ...
    //try {
      const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
      const transactionReceipt = await OptimisationGasTestInstance.transferETRMethod( _to[index-1], _amount[index-1]* 100, {from: App.account}
    ).on("transactionHash", hash => {
        console.log("transaction hash", hash);
    ...
    $('#ethAverage').text(gas);
    //} catch(error) {
    //console.log("error transferETRMethod");
    //}

  },
  transferConsenSys: async () => {
    $('#cSys').empty();
    $('#cSysAverage').empty();
    var index = Math.floor((Math.random() * 1000) + 1);
    console.log(index);
    ...
    //try {
      const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
      const transactionReceipt = await OptimisationGasTestInstance.transferConsenSys(_to[index-1], _amount[index-1]* 100, {from: App.account}
    ).on("transactionHash", hash => {
        console.log("transaction hash", hash);
    ...
    console.log("gasUsed in this transaction: ", gas );
    $('#cSys').text(gas*1000);
    $('#cSysAverage').text(gas);
  //  } catch(error) {
    //console.log("error transferConsenSys");
    //}
  },
  sendBatch: async (nrAddr) => {
    var c;
    if (nrAddr<=10){
      c=10;
    }else if(nrAddr>10 && nrAddr<=100){
      c=100;
    ...
    var batch = '#batch'+c;
    var batchAverage = '#batchAverage'+c;
    $(batch).empty();
    $(batchAverage).empty();

    var _adresandBalanc = await App.getAddresses(nrAddr);
    var _from = App.account
    var _to = _adresandBalanc[0];
    console.log(_to);
    var _amount = _adresandBalanc[1]
    ...
    //try {
      const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
      const transactionReceipt = await OptimisationGasTestInstance.sendBatch(_to,_amount,
        {
            from: App.account
    ...
    var gasUse = gas *(1000/nrAddr);
    var gasAvr = gas/nrAddr;
    $(batch).text(gasUse);
    $(batchAverage).text(gasAvr);
    //} catch(error) {
    //console.log("error sendBatch");
    //}
  },
  sendBatchCS: async (nrAddr) => {
    var c;
    if (nrAddr<=10){
      c=10;
    }else if(nrAddr>10 && nrAddr<=100){
...
console.log("sendBatch "+ nrAddr + "call");
    var batchCs = '#batchCs'+c;
    var batchCsAverage = '#batchCsAverage'+c;
    $(batchCs).empty();
    $(batchCsAverage).empty();
    ...
    //try {
      const OptimisationGasTestInstance = await  App.contracts.OptimisationGasTestArtifact.deployed();
      const transactionReceipt = await OptimisationGasTestInstance.sendBatchCS(_to,_amount,
        {
            from: App.account
